...
- Разработка код-стороны шла на Windows (без компилятора Swift), сборка и запуск — на Mac
  ...
  локально собрать.
- Сам Xcode-проект (`.xcodeproj`, `Assets.xcassets` и т.д.) физически лежит на Mac в
  `Kopeyka2/Kopeyka/` (имя `Kopeyka2` — исторический артефакт из-за коллизии имён при
  создании проекта) и **осознанно не в git** — в `.gitignore` есть `Kopeyka2/`. В репозитории
  только исходники (`Kopeyka/`). Если `git add -A`/Source Control в Xcode случайно застейджит
  `Kopeyka2/` — просто `git restore --staged Kopeyka2/`, коммитить его не нужно.
...
└── Shared/
    ...
    ├── Formatters/RelativeDateFormatter.swift — «Сегодня, 14:32» / «Вчера, 09:15» / «3 авг, 18:20»
...
- **Дата и время в истории** — у Expense/Income дата хранилась с самого начала (`date`),
  но в строках истории не выводилась. Теперь под категорией/источником показывается
  «Сегодня, 14:32» / «Вчера, 09:15» / «3 авг, 18:20» (`RelativeDateFormatter`)
